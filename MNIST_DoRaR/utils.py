...
class save_batch(object):
    ...
    def __init__(self, dataset, batch, label, label_pred,
                 label_approx, index,
                 filename, is_cuda, color = 'gray', word_idx = None):
    ...
    def imdb(self):
        ...
        for i in range(self.batch.size(0)):
            ...
            for j in range(num_line):
                ...
                QD1 = review[(width * j):(width * (j + 1))]
                list=[]
                for i in QD1:
                        list.append(i)

                QD2 = "".join(list)
                print(review[(width * j):(width * (j + 1))])
                print(QD2)
                draw.text((20, current_line), QD2, 'black')
    ...
    def mnist(self):
        ...
        for i in range(n_img):
            ...
            plt.imshow(img0.cpu(), cmap = 'autumn_r')
            # chunk selected
            img2 = img[i].view(-1)#.numpy()
            self.index = self.index.long()
            #print('selected chunk index ={}'.format(self.index[i]))
            if self.color != 'jet':
                img2[self.index[i]] = cuda(torch.tensor(float('nan')), self.is_cuda)
            img2 = img2.view(img0.size())
            plt.title('True {}, Apx {}'.format(self.label[i], self.label_approx[i]))
            plt.imshow(img2.cpu(), cmap = self.color)
    ...
    def cifar10(self):
        ...
        for i in range(n_img):
            ...
            if img.size(1) == 3:
                plt.subplot(n_row, 2 * n_col, 1 + 2 * i)
            else:
                plt.subplot(n_row, n_col, 1 + i)
            plt.axis('off')
            # original image
            img0 = img[i].squeeze(0)  # .numpy()
            if img0.size(0) == 3:
                img0 = img0.swapaxes(0, 1)
                img0 = img0.swapaxes(1, 2)
                plt.title('True {}, Apx {}'.format(self.label[i], self.label_approx[i]))
                plt.imshow(img0.cpu())
            else:
                plt.imshow(img0.cpu(), cmap='autumn_r')
            # chunk selected
            if img.size(1) == 3:
                plt.subplot(n_row, 2 * n_col, 2 + 2 * i)
                plt.axis('off')

            if img.size(1) == 3:
                img2=cuda(torch.zeros([32*32]), self.is_cuda)
                img2[self.index[i].long()] = cuda(torch.tensor(float(1)), self.is_cuda)
                img2 = img2.view(32,32)
            else:
                img2 = img[i].view(-1)  # .numpy()
                self.index = self.index.long()
                # print('selected chunk index ={}'.format(self.index[i]))
    ...
    def baseline_synthetic(self):
        ...
        for i in range(n_img):
            ...
            # original image
            img0 = img[i].squeeze(0)  # .numpy()
            plt.plot(img0[0,:].cpu(), img0[1,:].cpu(), 'b.')
            # chunk selected
            img2 = img[i].squeeze(0)  # .numpy()
            self.index = self.index.long()
            # print('selected chunk index ={}'.format(self.index[i]))
            # if self.color != 'jet':
            img2[:, self.index[i]] = cuda(torch.tensor(float('nan')), self.is_cuda)
            img2 = img2.view(img0.size())
            plt.title('True {}, Apx {}'.format(self.label[i], self.label_approx[i]))
            plt.plot(img2[0,:].cpu(), img2[1,:].cpu(), 'r.')
            fig.savefig(str(self.filename) + '_' + str(i) + '.png')

        # fig.subplots_adjust(wspace=0.05, hspace=0.35)
        # fig.savefig(str(self.filename))
        #
        # x = img0[0, 0 + 164 * 6:163 + 164 * 6].cpu().numpy()
        # y = img0[1, 0 + 164 * 6:163 + 164 * 6].cpu().numpy()
        # fig = plt.figure(figsize=(n_col * 1.5, n_row * 1.5))
        # plt.plot(x, y, 'ro')
        # fig.savefig(str(self.filename))
